...
import { updateLeasePropertyById } from "@/services/leaseProperty";
...
const formSchema = z.object({
  ...
  leasePrice: z.coerce
    .number()
    .positive("Lease price must be greater than 0"),
  leaseDuration: z.coerce
    .number()
    .positive("Lease duration (months) must be greater than 0"),
});

export default function EditLeasePropertyForm({
  leaseProperty,
}: {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      propertyType: leaseProperty.propertyType,
      description: leaseProperty.description,
      numOfTenants: leaseProperty.numOfTenants,
      leasePrice: leaseProperty.leasePrice,
      leaseDuration: leaseProperty.leaseDuration,
    },
  });
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      await updateLeasePropertyById(leaseProperty.leasePropertyId, values);
      toast({
        title: "Success!",
        description: "Lease property successfully edited",
      });
      setOpen(false);
      form.reset();
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-[600px] max-h-[500px] overflow-scroll">
        <DialogHeader>
          <DialogTitle>Edit Lease Property</DialogTitle>
          <DialogDescription>
            Make changes to your lease property
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form
          ...
          >
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Monthly Lease Price</FormLabel>
                  <FormDescription>In XTokens</FormDescription>
                  <FormControl>
                    <Input
